...
impl Tool for StatusTool {
    fn draw(&mut self, ui: &Ui, io: &Io, state: &mut GuiState) {
        ...
        macro_rules! status_flag {
            ($name:expr, $tooltip:expr, $location:expr) => {{
                ...
                state.computer.registers.r_status =
                    set_bit_at!(state.computer.registers.r_status, $location, updated);
        ...
        status_flag!(
            "Знак (N)",
            "Сообщает, что в регистре БР отрицательное число",
            2
        );
        ...
        status_flag!(
            ...
            "Когда этот бит установлен, ВУ могут вызвать прерывание ЭВМ",
            4
        );
